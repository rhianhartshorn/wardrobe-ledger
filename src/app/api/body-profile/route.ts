...
export function profileToContext(p: BodyProfile): string {
  ...
  if (p.fitPreference) {
    const f = { relaxed: 'prefers relaxed, easy-fitting clothes', tailored: 'prefers structured, tailored pieces', mix: 'comfortable with both relaxed and tailored fits' }[p.fitPreference];
    parts.push(f);
  }
  if (p.undertone) {
    const u = { warm: 'warm skin undertone (golden/peachy/olive)', cool: 'cool skin undertone (pink/rosy/blue-toned)', neutral: 'neutral skin undertone' }[p.undertone];
    parts.push(u);
  }
  if (p.hairTone) {
    const hair = { light: 'light/blonde hair', medium: 'medium brown hair', dark: 'dark brown or black hair', red: 'red or auburn hair', grey: 'grey or silver-white hair' }[p.hairTone];
    parts.push(hair);
  }
