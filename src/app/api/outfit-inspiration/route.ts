...
export async function POST(req: NextRequest) {
  if (!GOOGLE_API_KEY) {
    return NextResponse.json({ error: 'Inspiration images not configured — add GOOGLE_API_KEY to environment variables.' }, { status: 503 });
  }

  try {
    const body = await req.json() as { items?: SlimItem[]; styleReference?: string; stylingNote?: string };
    ...
    const referenceLine = body.styleReference ? ` Aesthetic reference: ${body.styleReference}.` : '';
    const stylingLine = body.stylingNote ? ` Styling detail to render exactly: ${body.stylingNote}.` : '';

    const parts: unknown[] = [
      ...garmentParts.map((g) => ({ inline_data: g })),
      { text: `${STYLIST_PERSONA}\n\nThe attached images are real clothing items: ${outfitDesc}. Generate a single photorealistic editorial-style image of an anonymous, generic model wearing all of these items together as a complete, well-styled outfit — this is a style inspiration reference, not a depiction of any real specific person. Clean composition, flattering natural light, the garments fitting and falling as they would on a real body.${referenceLine}${stylingLine}` },
    ];
    ...
    logExternalCall({ ts: Date.now(), route: 'outfit-inspiration', model: 'gemini-flash-image' }).catch(() => {});
