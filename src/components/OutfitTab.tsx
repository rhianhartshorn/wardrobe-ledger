...
import { compressImage, colorDot, slim, buildWearBehaviourSummary, buildWardrobeGrid } from './utils';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/outfit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items: slim(items), weather, occasion, note, profileImageFilename, bodyProfile, topWorn, savedLookTitles, workedLookTitles, didntWorkLookTitles, wearBehaviourSummary, wardrobeGrid: grid?.base64, wardrobeGridMapping: grid?.mapping }),
      });
